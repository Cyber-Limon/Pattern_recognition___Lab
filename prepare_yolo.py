...
def get_train_test_image():
    ...
    all_images = [f.replace('.jpg', '') for f in os.listdir(jpegimages_path)]
    ...
    for image_name in all_images:
        ...
        try:
            ...
            if classes_in_image:
                main_class = Counter(classes_in_image).most_common(1)[0][0]
                image_classes[image_name] = main_class
            else:
                image_classes[image_name] = None

        except Exception as e:
            print(f"Ошибка чтения {xml_path}: {e}")
            image_classes[image_name] = None

    valid_images = [img for img, cls in image_classes.items() if cls is not None]
    valid_classes = [image_classes[img] for img in valid_images]

    train_images, test_images = train_test_split(valid_images,
                                                 test_size=0.2,
                                                 random_state=42,
                                                 stratify=valid_classes)

    print(f"--- Подготовка изображений ---")
    print(f"- Всего: {len(all_images)}")
    print(f"- Валидные: {len(valid_images)}")
    print(f"- Тренировочные: {len(train_images)}")
    print(f"- Тестовые: {len(test_images)}")
...
def build_yolo_target(annotation_path, image_name):
    ...
    target = np.zeros((grid_h, grid_w, num_anchors, 5))
