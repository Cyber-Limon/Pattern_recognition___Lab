...
def extract_pred_boxes(pred_data):
    pred_boxes = []
    ...
    for i in range(grid_size[0]):
        for j in range(grid_size[1]):
            for a in range(num_anchors):
                if pred_reshaped[i, j, a, 4] > confidence_thresholds[1]:
                    ...
                    pred_confidence = pred_reshaped[i, j, a, 4]

                    pred_boxes.append({'bbox': [x1, y1, x2, y2], 'confidence': pred_confidence})
...
def evaluate(model, test_images, test_true_boxes):
    ...
    for i in range(len(test_images)):
        ...
        for true_box in true_boxes:
            ...
            if best_iou > confidence_thresholds[1] and best_pred_idx != -1:
                true_positives += 1
